...
//                                                e.printStackTrace();
     ...
     * @return
     */
    public final static List<Class> getServiceClass(Context context, String packageName, List<Class> excludeList) {

        List<Class> returnClassList = new ArrayList<Class>();
        try {
            //Get all service classes in the AndroidManifest.xml
            PackageInfo packageInfo = context.getPackageManager().getPackageInfo(packageName, PackageManager.GET_SERVICES);
            if (packageInfo.services != null) {
//                Log.d(TAG, "Found " + packageInfo.activities.length + " service in the AndrodiManifest.xml");
...
//                                                e.printStackTrace();
                ...
                Log.d(TAG, "Filter out, left " + returnClassList.size() + " service," + Arrays.toString(returnClassList.toArray()));

                //Exclude some service classes
                if (excludeList != null) {
                    returnClassList.removeAll(excludeList);
                    Log.d(TAG, "Exclude " + excludeList.size() + " service," + Arrays.toString(excludeList.toArray()));
                }
                Log.d(TAG, "Return " + returnClassList.size() + " service," + Arrays.toString(returnClassList.toArray()));
            }
        } catch (PackageManager.NameNotFoundException e) {
            e.printStackTrace();
        }
